...
async function main() {
  try {
    const loginRes = await new Promise((resolve, reject) => {
      const req = http.request(loginOptions, (res) => { let d = ''; res.on('data', c => d += c); res.on('end', () => resolve({ statusCode: res.statusCode, data: d, headers: res.headers })); });
    ...
    const gstRes = await new Promise((resolve, reject) => {
      const req = http.request(gstOptions, (res) => { let d = ''; res.on('data', c => d += c); res.on('end', () => resolve({ statusCode: res.statusCode, data: d })); });

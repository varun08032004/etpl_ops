...
async function main() {
  try {
    ...
    console.log('\nGST Collected:', gstRes.statusCode);
    console.log(JSON.parse(gstRes.data));

    // Test GST Liability
    const liabilityOptions = { hostname: '127.0.0.1', port: 5001, path: '/api/accounting/reports/gst-liability?from=2026-07-01&to=2026-08-31', method: 'GET', headers: { 'Authorization': 'Bearer ' + token } };
    const liabilityRes = await makeRequest(liabilityOptions);
    console.log('\nGST Liability:', liabilityRes.statusCode);
    console.log(JSON.parse(liabilityRes.data));

    // Test Platform Settlement
    const settlementOptions = { hostname: '127.0.0.1', port: 5001, path: '/api/accounting/reports/platform-settlement?from=2026-07-01&to=2026-08-31', method: 'GET', headers: { 'Authorization': 'Bearer ' + token } };
    const settlementRes = await makeRequest(settlementOptions);
    console.log('\nPlatform Settlement:', settlementRes.statusCode);
    console.log(JSON.parse(settlementRes.data));

...
(async () => {
  ...
  console.log('August P&L query results:');
  for (const a of rows) {
    const d = Number(a.total_debit);
    const c = Number(a.total_credit);
    const net = a.account_type === 'income' ? c - d : d - c;
    if (net !== 0) {
      console.log(`  ${a.code} ${a.name} (${a.account_type}): Dr=${d} Cr=${c} Net=${net}`);
    }
  }
  
  // Direct P&L query for July

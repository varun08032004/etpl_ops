...
// Initialize Slack App only if all required env vars are present
function initSlackApp() {
  const signingSecret = process.env.SLACK_SIGNING_SECRET;
  const botToken = process.env.SLACK_BOT_TOKEN;
  const appToken = process.env.SLACK_APP_TOKEN;

  if (!signingSecret || !botToken || !appToken) {
    console.log('[slackBot] Missing Slack credentials — bot not started');
    console.log('[slackBot] Required: SLACK_SIGNING_SECRET, SLACK_BOT_TOKEN, SLACK_APP_TOKEN');
    return null;
  }
  ...
  try {
    ...
    console.log('[slackBot] Slack bot initialized successfully');
    return app;
  } catch (err) {
    console.error('[slackBot] Failed to initialize:', err.message);
    return null;
  }
}

// Initialize on module load
slackApp = initSlackApp();
...
// ──────────────────────────────────────────────────────────────────────────
// Register commands only if slackApp is initialized
// ──────────────────────────────────────────────────────────────────────────
function registerCommands() {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // Command: /etpl-help - Show all commands
  // ──────────────────────────────────────────────────────────────────────────
...
// Register commands if slackApp is available
if (slackApp) {
  registerCommands();
}
...
async function startSlackBot() {
  if (!slackApp) {
    console.log('[slackBot] Slack bot not initialized — missing credentials');
    return null;
  }

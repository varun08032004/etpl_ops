...
// Requires a reason — a voided document leaves a gap in any auto-sequenced
// number (e.g. SC-0002 voided, SC-0003 issued as the correction), and that
// gap should always be explainable later, not just a silent hole in the
// numbering.
router.post('/generated/:id/void', requireRole('admin', 'hr', 'finance'), async (req, res) => {
  try {
    const { reason } = req.body;
    if (!reason || !reason.trim()) return res.status(400).json({ error: 'A reason is required to void a document' });

    const { rows: [updated] } = await safeQuery(
      `UPDATE generated_documents
       SET status = 'void', void_reason = $1, voided_by = $2, voided_at = NOW()
       WHERE id = $3 RETURNING *`,
      [reason.trim(), req.staff.id, req.params.id]
    );
    if (!updated) return res.status(404).json({ error: 'Document not found' });
    await auditLog.logAction({ staffId: req.staff.id, action: 'document.voided', entity: 'generated_documents', entityId: updated.id, newValue: { reason: reason.trim() } });
    res.json({ document: updated });
  } catch (err) {

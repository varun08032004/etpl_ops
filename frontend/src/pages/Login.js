...
function Field({ icon: Icon, label, labelAccent, error, children }) {
  return (
    <div className="mb-3.5">
      ...
      <div className="relative">
        {Icon && (
          <Icon className="pointer-events-none absolute left-4 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-white/35" />
        )}
        {children}
      </div>
    </div>
...
const inputBase =
  'h-[46px] w-full rounded-xl border border-white/10 bg-black/40 pl-11 pr-11 text-[15px] text-white ' +
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button
        ...
        className="mt-3 w-full rounded-xl py-3 text-[13.5px] font-medium text-white/50 transition-colors hover:text-white"
      >
        Back to sign in
      </button>
    </form>
    ...
    <AnimatePresence>
      {error && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0 }}
          className="mb-4 rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-3 text-[13px] text-red-400"
        >
          {error}
        </motion.div>
      )}
...
export default function Login() {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-[#050807] font-sans text-white flex flex-col">
      ...
      <div className="relative mx-auto flex w-full max-w-[1600px] flex-1 min-h-0 flex-col items-stretch justify-center px-6 lg:flex-row lg:px-10 lg:py-6">

        {/* ============================= LEFT PANEL ============================= */}
        <div className="relative flex min-h-0 w-full flex-col justify-between lg:w-[55%] lg:pr-14 lg:py-2">
          <div>
            {/* Logo */}
            <motion.div
        ...
        {/* ============================= RIGHT PANEL ============================= */}
        <div className="flex w-full min-h-0 items-center justify-center py-4 lg:w-[45%] lg:py-2">
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.15 }}
            className="relative w-full max-w-[440px] rounded-3xl border border-white/[0.08] bg-[#0F1313]/90
                       p-6 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.6)] backdrop-blur-sm sm:p-7"
          >

            {/* Secure badge */}
            <div className="absolute right-6 top-6 flex items-center gap-1.5 rounded-full border border-[#22C55E]/25
                            bg-[#22C55E]/10 px-2.5 py-1 text-[10.5px] font-medium text-[#4ADE80] sm:right-7 sm:top-7">
            ...
            <AnimatePresence mode="wait">
              {forgotMode ? (
                ...
                <motion.form key="2fa" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleTwoFactorSubmit}>
                ...
                <motion.form key="device" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleVerifyDevice}>
                  <p className="mb-5 text-[13.5px] leading-relaxed text-white/50">
                ...
                <motion.form key="login" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleSubmit}>
                  ...
                  <div className="-mt-1.5 mb-3.5 text-right">
                    <button
                      ...
                      className="text-[13px] font-medium text-white/50 transition-colors hover:text-[#22C55E]"
                    >
                      Forgot password?
                    </button>
                  </div>
            ...
            <div className="mt-3.5 flex justify-center">
              <div className="flex h-8 w-8 items-center justify-center rounded-full border border-[#22C55E]/25 bg-[#22C55E]/5">
      ...
      <div className="relative mx-auto flex w-full max-w-[1600px] shrink-0 flex-col items-center justify-between gap-1.5 border-t
                      border-white/[0.06] px-6 py-3 text-[11.5px] text-white/35 sm:flex-row lg:px-10">
